...
import {
  ...
  DEUCES_PAYTABLE, HandRank, HAND_RANK_NAME, Rank,
...
export function DeucesWildClient() {
  ...
  function draw() {
    ...
    const result = applyHolds(round, holds, DEUCES_PAYTABLE, bet);
  ...
  // Highlight resolution for Deuces:
  //   - on a winning draw, every card glows "win"
  //   - while in "dealt" phase, held cards show the "hold" outline
  //   - any face-up 2 in dealt-or-drawn state shows the "wild" accent
  //     (so the player remembers 2s substitute as any rank). The wild
  //     glow is overridden by win-flash and hold-outline since those
  //     are stronger phase-relevant signals.
  const displayHighlights = displayCards.map((card, i): "hold" | "win" | "wild" | null => {
    ...
    if (card != null && card.rank === Rank.TWO && phase !== "idle") return "wild";
    return null;
  });
      ...
      <div className="cabinet-vp-wrap">
        ...
        <div className="cabinet-vp cabinet-vp-shell">
          ...
          <div className="flex-none border-b border-[var(--cabinet-vp-border)]">
            <PaytablePanel
              paytable={DEUCES_PAYTABLE}
              bet={bet}
              topTierRank={HandRank.NATURAL_ROYAL_FLUSH}
