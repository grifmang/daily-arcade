// RNG contract and implementations for the video-poker engine.
// Spec: docs/superpowers/specs/cards-video-poker-engine.md section 8
// Architecture: ARCHITECTURE.md section 15.5
//
// Intentional duplication of the slot RNG shape — both modules expose the
// same SlotRng interface so a future hoist to lib/util/rng.ts is mechanical.
// See ARCHITECTURE 15.5 for the YAGNI rationale.
...
// ---------------------------------------------------------------------------
// xoshiro256** — seedable, deterministic. Used by tests and any deterministic
// fixture generation. Same algorithm family as lib/seed.ts (the daily-puzzle
// PRNG) but a separate, isolated instance — cards and daily puzzles never
// share a PRNG.
// ---------------------------------------------------------------------------
...
// per-hand outcomes. Per ADR-C5 (DECISIONS.md 2026-05-01), card games do NOT
// consume the daily seed engine; this RNG is fresh per hand.
// ---------------------------------------------------------------------------
...
export function createCryptoRng(): SlotRng {
  ...
  return {
    ...
    nextInt(maxExclusive: number): number {
      ...
      // true and the loop infinite. Card-shuffle's Fisher-Yates calls `nextInt(32)`,
      // `nextInt(16)`, etc., which would have triggered this. The slot RNG has
      // the same bug latent (it only calls nextInt(60) so it dodges it).
